...
const ProductsManager = () => {
  ...
  const handleToggleFeatured = async (productId, currentValue) => {
    try {
      ...
      if (data.success) {
        // Update local state
        setProducts(prev => prev.map(p => p._id === productId ? { ...p, featured: !currentValue } : p))
      } else {
        alert('Failed to update featured status: ' + (data.message || 'Unknown'))
      }
    } catch (error) {
  ...
  const handleToggleShowOnHero = async (product) => {
    try {
      const token = localStorage.getItem('adminToken')
      ...
      if (allData.success && Array.isArray(allData.data)) {
        existing = allData.data.find(s => s.productId === product._id)
      }

      if (!existing) {
        // Create a new carousel slide for this product
        const payload = {
        ...
        if (data.success) {
          // Show success and mark in UI
          setProducts(prev => prev.map(p => p._id === product._id ? { ...p, showOnHero: true } : p))
        } else {
          alert('Failed to add to hero: ' + (data.message || 'Unknown'))
        }
      } else {
        ...
        const data = await res.json()
        if (data.success) {
          setProducts(prev => prev.map(p => p._id === product._id ? { ...p, showOnHero: false } : p))
        } else {
          alert('Failed to remove from hero: ' + (data.message || 'Unknown'))
        }
      }
    } catch (error) {
      console.error('Error toggling hero slide:', error)
      alert('Error toggling hero slide: ' + error.message)
    }
  }
              ...
              <div className="product-info">
                ...
                <div className="product-actions">
                  ...
                  <button 
                    ...
                    className={`toggle-featured ${product.featured ? 'active' : ''}`}
                    title={product.featured ? 'Unfeature product' : 'Feature product'}
                  >
                    {product.featured ? '★ Featured' : '☆ Feature'}
                  </button>
                  <button
                    onClick={() => handleToggleShowOnHero(product)}
                    className={`toggle-hero ${product.showOnHero ? 'active' : ''}`}
                    title={product.showOnHero ? 'Remove from hero' : 'Show on hero'}
                  >
                    {product.showOnHero ? 'Hide from Hero' : 'Show on Hero'}
                  </button>
                </div>
              </div>
            </motion.div>
